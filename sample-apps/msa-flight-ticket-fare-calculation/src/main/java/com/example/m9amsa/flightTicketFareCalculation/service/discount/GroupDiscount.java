...
public class GroupDiscount implements DiscountCalculator {

    /**
     * {@inheritDoc}
     * 
     * <pre>
     * グループ割では、搭乗クラス加算額とピーク時期積算比率から求める基本運賃からグループ割の割引額を引いた金額を使用します。
     * </pre>
     */
    @Override
     ...
     * この計算ロジックの対象割引を取得します。
     * 
     * @return {@code DiscountType.GROUP}。
     */
    @Override
    public DiscountType targetDiscountId() {
        return DiscountType.GROUP;
    }
     ...
     * @return 搭乗日60日前～搭乗日前日に当たればtrueを返します。
     */
    @Override
    public boolean isAvailableTravelDate(LocalDate travelDate) {
        ...
        // 搭乗日60日前～搭乗日前日
        return (1 <= duration.toDays()) && (duration.toDays() <= 60);
    }
     ...
     * @return 三人以上であればtrueと返します。
     */
    @Override
    public boolean isAvailableTotalPassengers(Integer totalPassengers) {
        return (totalPassengers >= 3);
    }
}
