 ...
 * レディース割を計算します。
 * 
 * サービスでは区間、人数、時期の判断のみを用いて該当割引の判定を行いますので、クライアント側で女性かどうかの判断をしてください。
 * 
 */
@Component("ladiesDiscount")
public class LadiesDiscount implements DiscountCalculator {

    /**
     * {@inheritDoc}
     * 
     * <pre>
     * レディース割では、搭乗クラス加算額とピーク時期積算比率から求める基本運賃からレディース割の割引額を引いた金額を使用します。
     * </pre>
     */
    @Override
     ...
     * この計算ロジックの対象割引を取得します。
     * 
     * @return {@code DiscountType.LADIES}。
     */
    @Override
    public DiscountType targetDiscountId() {
        return DiscountType.LADIES;
    }
