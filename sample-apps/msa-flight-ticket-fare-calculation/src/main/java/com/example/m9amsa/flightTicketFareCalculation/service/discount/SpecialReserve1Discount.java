 ...
 * 
 */
@Component("specialReserve1Discount")
public class SpecialReserve1Discount implements DiscountCalculator {

    /**
     * {@inheritDoc}
     * 
     * <pre>
     * 特別予約割では、搭乗クラス加算額とピーク時期積算比率から求める基本運賃から特別予約割の割引額を引いた金額を使用します。
     * </pre>
     */
    @Override
    public int calcFare(FareCalcInfo fareCalcInfo, SeatClassCharge seatClassCharge, PeakRatio peakRatio) {
        ...
        // 基本運賃 - 割引額(基本運賃の10％)
        return (int) (basicFare - Math.floor(basicFare * 0.1));
    }

    /**
     * この計算ロジックの対象割引を取得します。
     * 
     * @return {@code DiscountType.SPECIAL_RESERVE1}。
     */
    @Override
    public DiscountType targetDiscountId() {
        return DiscountType.SPECIAL_RESERVE1;
    }
