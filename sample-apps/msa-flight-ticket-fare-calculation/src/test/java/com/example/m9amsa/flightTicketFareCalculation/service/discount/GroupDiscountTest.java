...
public class GroupDiscountTest {
    ...
    public void testCalcFare() {
        DiscountCalculator discount = discountCalculatorFactory.getCalculator(DiscountType.GROUP);
        ...
        assertThat("割引価格が正しいこと", actFare, equalTo(34300));
    }
    ...
    public void testTargetDiscountId() {
        ...
        assertThat("割引タイプが正しいこと", discount.targetDiscountId(), equalTo(DiscountType.GROUP));
    }
    ...
    public void testIsAvailableTravelDate() {
        ...
        assertTrue("搭乗日30日前", actual);

        // 搭乗日60日前
        travelDate = LocalDate.now().plusDays(60);

        actual = discount.isAvailableTravelDate(travelDate);

        assertTrue("搭乗日60日前", actual);

        // 搭乗日61日前
        travelDate = LocalDate.now().plusDays(61);

        actual = discount.isAvailableTravelDate(travelDate);

        assertFalse("搭乗日61日前", actual);
    }
    ...
    public void testIsAvailableTotalPassengers() {
        ...
        Boolean actual = discount.isAvailableTotalPassengers(1);

        assertFalse("搭乗人数：1", actual);

        actual = discount.isAvailableTotalPassengers(2);

        assertFalse("搭乗人数：2", actual);

        actual = discount.isAvailableTotalPassengers(3);

        assertTrue("搭乗人数：3", actual);

        actual = discount.isAvailableTotalPassengers(4);

        assertTrue("搭乗人数：4", actual);
    }
}
