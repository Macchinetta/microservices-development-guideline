...
public class ApiExceptionHandlerTest {
    ...
    public void testHandleNoSuchElementException() {
        ResponseEntity<Object> actual = apiExceptionHandler.handleNoSuchElementException(new NoSuchElementException("test"),
                request);
        ...
        assertThat("Bodyのメッセージが正しいこと", responce.getMessage(), equalTo("Data access error"));
    ...
    public void testHandleHttpStatus404Exception() {
        ...
        assertThat("Bodyのインスタンスが正しいこと", actual.getBody(), instanceOf(ApiValidationErrorResponse.class));
        ApiValidationErrorResponse responce = (ApiValidationErrorResponse) actual.getBody();
        assertThat("Bodyのメッセージが正しいこと", responce.getMessage(), equalTo("Not Found"));
        assertTrue("Headersは空であること", actual.getHeaders().isEmpty());
    ...
    public void testHandleBusinessException() {
        ResponseEntity<Object> actual = apiExceptionHandler.handleBusinessException(new BusinessException(),
                request);

        assertThat("Bodyのインスタンスが正しいこと", actual.getBody(), instanceOf(ApiValidationErrorResponse.class));
        ApiValidationErrorResponse responce = (ApiValidationErrorResponse) actual.getBody();
        assertThat("Bodyのメッセージが正しいこと", responce.getMessage(), equalTo("Precondition Failed"));
        assertTrue("Headersは空であること", actual.getHeaders().isEmpty());
        assertThat("HttpStatusが正しいこと", actual.getStatusCode(), equalTo(HttpStatus.PRECONDITION_FAILED));
    }
