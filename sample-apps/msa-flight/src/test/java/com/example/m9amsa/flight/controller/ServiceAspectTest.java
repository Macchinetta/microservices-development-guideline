...
public class ServiceAspectTest {
    ...
    /**
     * correctReturnAirport()メソッドのテスト。
     * 
     */
    @Test
    public void testCorrectReturnForAirport() {
        MethodSignature methodSignature = Mockito.mock(MethodSignature.class);

        // ターゲットメソッドの戻り値がOptional<Airport>以外の場合
        Throwable e = catchThrowable(() -> {
            doReturn(String.class).when(methodSignature).getReturnType();
            doReturn(methodSignature).when(joinPoint).getSignature();

            serviceAspect.correctReturnAirport(joinPoint, (Service) null, (Optional<Airport>) null);
        });
        ...
        // ターゲットメソッドの戻り値がOptionalで、emptyではない場合
        reset(methodSignature);
        e = catchThrowable(() -> {
            Optional<Airport> airport = Optional.of(Airport.builder().id("HND").name("東京").build());

            doReturn(Optional.class).when(methodSignature).getReturnType();
            doReturn(methodSignature).when(joinPoint).getSignature();

            serviceAspect.correctReturnAirport(joinPoint, (Service) null, airport);
        });
        ...
        // ターゲットメソッドの戻り値がOptionalで、emptyの場合
        HttpStatus404Exception e404 = catchThrowableOfType(() -> {
            serviceAspect.correctReturnAirport(joinPoint, (Service) null, Optional.empty());
        }, HttpStatus404Exception.class);
