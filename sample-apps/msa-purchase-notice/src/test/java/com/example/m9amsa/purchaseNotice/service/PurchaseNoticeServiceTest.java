...
import com.example.m9amsa.purchaseNotice.externalmicroservice.service.FlightExternalMicroService;
...
public class PurchaseNoticeServiceTest {
    ...
    // 会員の場合
    @Test
    public void testPurchaseNoticeMember() throws Exception {

        Clock clock = Clock.fixed(Instant.parse("2019-10-31T12:00:00Z"), ZoneId.systemDefault());
        doReturn(clock).when(purchaseNoticeService).getClock();
        ...
        // メールアドレスが正しい場合
        RssRoot rssRoot = purchaseNoticeService.purchaseNotice("0001@ntt.com");
        ...
        assertThat("channelのdescriptionを確認する", rssRootAsXml, hasXPath("/rss/channel/description", equalTo("購入通知")));
        assertThat("itemのtitleを確認する", rssRootAsXml, hasXPath("/rss/channel/item/title", equalTo("予約Id：1の購入情報")));
        ...
        // メールアドレスが正しくない場合
        rssRoot = purchaseNoticeService.purchaseNotice("0001");
        assertThat("rssのversionを確認する", rssRootAsXml, hasXPath("/rss/@version", equalTo("2.0")));
        assertThat("channelのtitleを確認する", rssRootAsXml, hasXPath("/rss/channel/title", equalTo("M9AREF")));
        assertThat("channelのdescriptionを確認する", rssRootAsXml, hasXPath("/rss/channel/description", equalTo("購入通知")));
    }
    ...
    // 非会員の場合
    @Test
    public void testPurchaseNoticeNonMember() throws Exception {
        ...
        assertThat("itemのpubDateを確認する", rssRootAsXml, hasXPath("/rss/channel/item/pubDate",
                equalTo(LocalDateTime.now(clock).format(DateTimeFormatter.ofPattern("yyyy/MM/dd hh:mm")))));
        assertThat("itemのdescriptionを確認する", rssRootAsXml, hasXPath("/rss/channel/item/description", equalTo(
                "ご予約ありがとうございました。期限切れ前にお支払いください。予約の詳細メッセージです。\r\n予約Id：1\r\nフライト名：NTT01\r\n出発空港：HND - 東京(羽田)\r\n出発日：2019-05-07\r\n出発時刻：10:05\r\n到着空港：KIX - 大阪(関西)\r\n到着時刻：13:05\r\n搭乗クラス：N\r\n料金タイプ：片道\r\n料金：\\13500\r\n代表搭乗者：\r\n  お名前：渡辺太郎\r\n  年齢：31\r\n支払期限：2019-05-07\r\n")));
        ...
        AirportForEx airport = AirportForEx.builder().id(departureAirportId).name(airportMap.get(departureAirportId))
                .build();
        ...
        rssRoot = purchaseNoticeService.purchaseNotice("0001@ntt.com");
        assertThat("rssのversionを確認する", rssRootAsXml, hasXPath("/rss/@version", equalTo("2.0")));
        assertThat("channelのtitleを確認する", rssRootAsXml, hasXPath("/rss/channel/title", equalTo("M9AREF")));
        assertThat("channelのdescriptionを確認する", rssRootAsXml, hasXPath("/rss/channel/description", equalTo("購入通知")));
        assertThat("itemのtitleを確認する", rssRootAsXml, hasXPath("/rss/channel/item/title", equalTo("予約Id：1の購入情報")));
